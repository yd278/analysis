...
theorem Erdos_987 (z: ℕ → Circle) :
  ...
  lift C to ℝ using (by tauto); clear h this
  have hC' : (C:EReal) < (C+1:ℝ) := by norm_cast; linarith
  rw [←hC] at hC'; clear hC
  replace hC' := eventually_lt_of_limsup_lt hC'
  rw [eventually_atTop] at hC'
  choose k₀ hk₀ using hC'
  replace hk₀ : ∀ k ≥ k₀, ∃ N, ∀ n ≥ N, ‖∑ j ∈ .range n, ((z j)^k:ℂ)‖ < C+1 := by
    peel hk₀ with k hk hk₀
    replace hk₀ := eventually_lt_of_limsup_lt hk₀
    rw [eventually_atTop] at hk₀
    peel hk₀ with N hN n hn
    norm_cast at hn
  ...
  replace hN (K:ℕ) : ∃ w : ℕ → Circle, ∀ n, ∀ k ≤ K, k ≥ k₀ → ‖∑ j ∈ .range n, ((w j)^k:ℂ)‖ < 2*(C+1) := by
    ...
    calc
      ...
      _ ≤ ‖∑ j ∈ .range (N₀+n), ((z j)^k:ℂ)‖ + ‖∑ j ∈ .range N₀, ((z j)^k:ℂ)‖ :=
        norm_sub_le _ _
      _ < (C+1) + (C+1) := by gcongr <;> exact hN _ _ _ (by linarith)
      _ = _ := by ring
  generalize hC : 2*(C+1) = C'; rw [hC] at hN
  clear N z hC C
  have (n K:ℕ) : n*K^2 ≤ K^2 * (C')^2 + K*((2*k₀)*n^2) := by
    ...
    _ ≤ ∑ k ∈ .range K, ∑ k' ∈ .range K, ((C')^2 + (if k < k' + k₀ ∧ k' < k + k₀ then n^2 else 0)) := by
      ...
      . convert le_of_lt (hw n l (by omega) hl) with j hj
        field_simp; grind
      rw [←norm_conj, map_sum]
      convert le_of_lt (hw n l (by omega) hl) with j hj
      simp [←Circle.coe_inv_eq_conj]; field_simp; grind
    _ ≤ _ := by
      ...
      calc
        ...
        _ = _ := by simp
  clear hN
  obtain ⟨ n, hn ⟩ := exists_nat_gt ((C')^2 + 1)
  let K := 2*k₀*n^2 + 1
  specialize this n K
