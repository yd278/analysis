...
lemma Sequence.inf_of_unbounded_below (a:Sequence) (ha : ¬ a.BddBelow ) : a.inf = ⊥ := by
  ...
  . have := ge_inf (show a.m ≥ a.m by simp)
    ...
    simp at this
  contradiction
abbrev Sequence.odd_lt_even (a:Sequence) := ∀ n ≥ a.m , ∀ m ≥ a.m, Odd n → Even m → a n < a m
lemma Example_6_4_7.odd_lt_even : Example_6_4_7.odd_lt_even := by
  intro n hn m hm
  simp at hn hm
  lift n to ℕ using hn
  ...
  simp at hn hm
  simp
  have : (-1:ℝ) ^ n = -1 := by exact Odd.neg_one_pow hn
  simp[this]
  have : (-1:ℝ) ^ m = 1 := by exact Even.neg_one_pow hm
  simp[this]
  calc
...
lemma Example_6_4_7.even_antitone : Example_6_4_7.Even_antitone := by
  ...
  have : Even (n+2) := by
    apply Even.add hn
    simp
  have :(-1:ℝ) ^(n+2) = 1 := by exact Even.neg_one_pow this
  simp[this]
  have :(-1:ℝ) ^(n) = 1 := by exact Even.neg_one_pow hn
  simp[this]
...
lemma Example_6_4_7.odd_monotone : Example_6_4_7.odd_monotone := by
  ...
  have : Odd (n+2) := by
    ...
    simp
  have :(-1:ℝ) ^(n+2) = -1 := by exact Odd.neg_one_pow this
  simp[this]
  have :(-1:ℝ) ^(n) = -1 := by exact Odd.neg_one_pow hn
  simp[this]
lemma Sequence.even_antitone_extend (a:Sequence) (ha: a.Even_antitone) :
    ∀ n ≥ a.m, ∀ m ≥ n, Even n → Even m → a n ≥ a m := by
      intro n hn m hmn hne hme
...
abbrev Sequence.even_antitone_trans (a:Sequence) :=  ∀ n ≥ a.m, ∀ m ≥ n, Even n → Even m → a n ≥ a m 
lemma Example_6_4_7.even_antitone_trans : Example_6_4_7.even_antitone_trans := by
  intro n hn
  intro m hm hme
  have hea := even_antitone
  apply Example_6_4_7.even_antitone_extend hea n 
  simp
  simpa
  simpa
  simpa

lemma Sequence.odd_monotone_extend (a:Sequence) (ha: a.odd_monotone) :
    ∀ n ≥ a.m, ∀ m ≥ n, Odd n → Odd m → a n ≤ a m := by
      intro n hn m hmn hne hme
      suffices h: ∀ (i:ℕ), a n ≤ a (n + (i + i)) from by
        choose d hd0 hd using exists_nonneg_add_of_le hmn
        have hde: Even d := by
      ...
      have hvm : v ≥ a.m := by simp[v];linarith
      apply ha v hvm hve
abbrev Sequence.odd_monotone_trans (a:Sequence) :=  ∀ n ≥ a.m, ∀ m ≥ n, Odd n → Odd m → a n ≤ a m 
lemma Example_6_4_7.odd_monotone_trans : Example_6_4_7.odd_monotone_trans := by
  intro n hn
  intro m hm hme
  have hea := odd_monotone
  apply Example_6_4_7.odd_monotone_extend hea n 
  simp
  simpa
  simpa
  simpa

abbrev Sequence.even_upper (a:Sequence) := ∀ n ≥ a.m , ∀ m ≥ n , Even n → a n ≥ a m
lemma Sequence.even_upper_of_EA_and_OLE {a:Sequence} (hea : a.even_antitone_trans) (hole : a.odd_lt_even) : a.even_upper := by
  intro n hn m hm hne
  have hmm : m ≥ a.m := by linarith
  by_cases hme: Even m
  . apply hea _ hn _ hm hne hme
  simp at hme
  apply le_of_lt
  apply hole _ hmm _ hn hme hne

lemma Example_6_4_7.even_upper : Example_6_4_7.even_upper := by
  intro n hn m hm hne 
  apply Example_6_4_7.even_upper_of_EA_and_OLE even_antitone_trans odd_lt_even _ hn _ hm hne 

abbrev Sequence.odd_lower (a:Sequence) := ∀ n ≥ a.m , ∀ m ≥ n , Even n → a n ≥ a m
lemma Sequence.even_upper_of_EA_and_OLE {a:Sequence} (hea : a.even_antitone_trans) (hole : a.odd_lt_even) : a.even_upper := by
  intro n hn m hm hne
  have hmm : m ≥ a.m := by linarith
  by_cases hme: Even m
...
/-           simp[f] -/
/-           have :(-1:ℝ) ^ n = 1 := by exact Even.neg_one_pow hpn -/
/-           simp[this] -/
/-         rw[← hfn] -/
...
/-         simp[this] -/
/-         have : Even (n+1) := by exact Nat.even_add_one.mpr hpn -/
/-         have : (-1:ℝ)^(n+1) = 1 := by exact Even.neg_one_pow this -/
/-         simp[this] -/
/-         ring -/
/-       rw[← hfn] -/
/-       apply Sequence.sup_of_max -/
...
/-       rw[ show f m = Example_6_4_7 m by simp [f,hm]] -/
/-       lift m to ℕ using by linarith -/
/-       have hn : Even (n+1) := by exact Odd.add_one hpn -/
...
/-     specialize hr l 1 -/
/-     simp at hr -/
/-     have hε : l - 1 > 0 := by exact sub_pos.mpr hr -/
/-     set ε := l - 1 -/
...
/-           have : 0 ≤ (n:ℤ)+1:=by linarith -/
/-           simp[this] -/
/-           have : Odd (n+1) := by exact Even.add_one hpn -/
/-           have : (-1:ℝ)^(n+1) = -1 := by exact Odd.neg_one_pow this -/
/-           simp[this] -/
/-           ring -/
/-         rw[← hfn] -/
...
/-         rw[ show f m = Example_6_4_7 m by simp [f,hm]] -/
/-         lift m to ℕ using by linarith -/
/-         have hn : Odd (n+1) := by exact Even.add_one hpn -/
/-         rw[show (n:ℤ) + 1 = (n+1:ℕ) by rfl] -/
/-         by_cases hsp : m = n -/
...
/-       have hfn : f n = -(1 + (10:ℝ) ^ (-(n:ℤ) - 1)) := by -/
/-         simp[f] -/
/-         have :(-1:ℝ) ^ n = -1 := by exact Odd.neg_one_pow hpn -/
/-         simp[this] -/
/-       rw[← hfn] -/
/-       apply Sequence.inf_of_min -/
/-       . simp[f] -/
...
/-     apply EReal.coe_le_coe -/
/-     contrapose! hl -/
/-     have hε : -1 - l > 0 := by exact sub_pos.mpr hl -/
/-     set ε := -1 - l  -/
/-     choose N hN using Real.exists_decimal_seq hε  -/
...
/-   apply Sequence.sup_of_max -/
/-   . simp -/
/-   have : Even 0 := by exact Nat.even_iff.mpr rfl -/
...
/-   apply Sequence.inf_of_min -/
/-   . simp -/
/-   have ho1 : Odd 1 := by exact Nat.odd_iff.mpr rfl -/
