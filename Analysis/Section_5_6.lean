...
theorem Real.root_mul {x y:Real} (hx: x ≥ 0) (hy: y ≥ 0) {n:ℕ} (hn: n ≥ 1) : (x*y).root n = (x.root n) * (y.root n) := by
  ...
  rw[← mul_pow] at this
  field_simp at this
  obtain (heq|hpos) := hrx.eq_or_lt 
  . simp[← heq] at this ⊢ 
    rw[zero_pow  (by omega)] at this
    rwa[pow_eq_zero_iff (by omega)] at this
  obtain (heq| hposy):= hry.eq_or_lt
  . simp[← heq] at ⊢ this
    rw[zero_pow  (by omega)] at this
    rwa[pow_eq_zero_iff (by omega)] at this
  obtain (heq| hposxy):= hrxy.eq_or_lt
  . simp[← heq] at ⊢ this 
    rw[zero_pow (by omega)] at this
    symm at this
    rw[pow_eq_zero_iff (by omega)] at this
    nlinarith
  simp only[← pow_eq_pow] at this
  apply zpow_inj hposxy (mul_pos hpos hposy) (by omega) this
...
theorem Real.pow_cancel {y z:Real} (hy: y > 0) (hz: z > 0) {n:ℕ} (hn: n ≥ 1)
  (h: y^n = z^n) : y = z := by
    simp only[← pow_eq_pow] at h
    apply zpow_inj hy hz (by omega) h
...
theorem Real.ratPow_add {x:Real} (hx: x > 0) (q r:ℚ) : x^(q+r) = x^q * x^r := by
  ...
  rw[show (qa / qb:ℚ) = ((qa * rb:ℤ):ℚ) / ((qb * rb:ℕ):ℚ ) by qify;field_simp,
    ratPow_def hx _ (by apply mul_pos hqb hrb)
  ] 
  rw[show (ra / rb:ℚ) = ((qb * ra:ℤ):ℚ) / ((qb * rb:ℕ):ℚ ) by qify;field_simp,
    ratPow_def hx _ (by apply mul_pos hqb hrb)
  ] 
...
theorem Real.ratPow_ratPow {x:Real} (hx: x > 0) (q r:ℚ) : (x^q)^r = x^(q*r) := by
  ...
  rw[ show (qa / qb:ℚ ) * (ra / rb:ℚ) = (qa * ra :ℤ) / (qb * rb:ℕ) by qify;field_simp,
    ratPow_def hx _ (by apply mul_pos hqb hrb)
  ]
  rw[← zpow_mul]
  congr
...
theorem Real.ratPow_neg {x:Real} (hx: x > 0) (q:ℚ) : x^(-q) = 1 / x^q := by
  obtain ⟨qa, qb, hqb, rfl⟩ := Rat.eq_quot q 
  have : -(qa / qb :ℚ) = ((-qa:ℤ) / qb:ℚ)  := by
    norm_cast
    exact Rat.neg_divInt qa ↑qb
  rw[this]
