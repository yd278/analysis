...
theorem Real.max_mul (x y :Real) {z:Real} (hz: z.IsPos) : max (x * z) (y * z) = max x y * z := by
  ...
  . 
    ...
    have hxyz : y * z ≤ x * z := by
      exact (mul_le_mul_iff_of_pos_right hz).mpr h
    simp[hxyz,h]
  . 
    observe : ¬ y ≤ x
    observe hz : z > 0
    have hxyz : ¬ y * z ≤ x * z := by
      simp
      (expose_names; exact mul_lt_mul_right h2 hz_1)
    simp[hxyz,this]
  simp[h3]
...
theorem Real.min_mul (x y :Real) {z:Real} (hz: z.IsPos) : min (x * z) (y * z) = min x y * z := by
  ...
  . 
    ...
    have hxyz : ¬ x * z ≤ y * z := by
      simp
      (expose_names; exact mul_lt_mul_right h1 hz_1)
    simp[hxyz,h]
  . 
    observe : x ≤ y
    observe hz : z > 0
    have hxyz :  x * z ≤ y * z := by
      exact (mul_le_mul_iff_of_pos_right hz).mpr this
    simp[hxyz,this]
  simp[h3]
